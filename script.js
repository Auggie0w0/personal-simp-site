...
function voteForCharacter(characterId, voteType) {
    ...
    const userKey = `user_${Date.now().toString(36)}_${Math.random().toString(36).substr(2, 9)}`;
    
    if (!userVotes[characterId]) {
        userVotes[characterId] = userKey;
        localStorage.setItem('userVotes', JSON.stringify(userVotes));
    } else {
        // User has already voted for this character
        showVoteMessage('You have already voted for this character!', 'warning');
        return;
    }
    
    // Add the vote
    character.votes[voteType]++;
    character.votes.total++;
    
    // Calculate percentages
    const totalVotes = character.votes.total;
    const likePercent = totalVotes > 0 ? Math.round((character.votes.like / totalVotes) * 100) : 0;
    const lovePercent = totalVotes > 0 ? Math.round((character.votes.love / totalVotes) * 100) : 0;
    const favoritePercent = totalVotes > 0 ? Math.round((character.votes.favorite / totalVotes) * 100) : 0;
    
    // Save to localStorage
    saveCharacters();
    
    // Update the display
    updateVoteDisplay(characterId, character.votes, likePercent, lovePercent, favoritePercent);
    
    // Show success message
    const voteMessages = {
        like: 'Thanks for liking this character! 💕',
        love: 'You love this character! ❤️',
        favorite: 'This is now your favorite! ⭐'
    };
    showVoteMessage(voteMessages[voteType], 'success');
}

function updateVoteDisplay(characterId, votes, likePercent, lovePercent, favoritePercent) {
    const voteElement = document.getElementById(`votes-${characterId}`);
    if (voteElement) {
        voteElement.innerHTML = `
            <div class="vote-stats">
                <div class="vote-total">
                    <span class="vote-count">${votes.total}</span>
                    <span class="vote-label">Total Votes</span>
                </div>
                <div class="vote-breakdown">
                    <div class="vote-category">
                        <span class="vote-icon">👍</span>
                        <span class="vote-number">${votes.like}</span>
                        <span class="vote-percent">(${likePercent}%)</span>
                    </div>
                    <div class="vote-category">
                        <span class="vote-icon">❤️</span>
                        <span class="vote-number">${votes.love}</span>
                        <span class="vote-percent">(${lovePercent}%)</span>
                    </div>
                    <div class="vote-category">
                        <span class="vote-icon">⭐</span>
                        <span class="vote-number">${votes.favorite}</span>
                        <span class="vote-percent">(${favoritePercent}%)</span>
                    </div>
                </div>
            </div>
            <div class="vote-buttons">
                <button onclick="voteForCharacter('${characterId}', 'like')" class="vote-btn like-btn">
                    <span class="vote-btn-icon">👍</span>
                    <span class="vote-btn-text">Like</span>
                </button>
                <button onclick="voteForCharacter('${characterId}', 'love')" class="vote-btn love-btn">
                    <span class="vote-btn-icon">❤️</span>
                    <span class="vote-btn-text">Love</span>
                </button>
                <button onclick="voteForCharacter('${characterId}', 'favorite')" class="vote-btn favorite-btn">
                    <span class="vote-btn-icon">⭐</span>
                    <span class="vote-btn-text">Favorite</span>
                </button>
            </div>
        `;
    }
...
// Legacy rating system (keeping for backward compatibility)
    ...
    <main class="main-content">
        <div class="character-profile">
            ...
            <div class="character-content">
                ...
                <div class="content-section">
                    <h2>Vote for this Character</h2>
                    <div id="votes-${character.id}" class="voting-section">
                        <div class="vote-stats">
                            <div class="vote-total">
                                <span class="vote-count">${(character.votes?.total || 0)}</span>
                                <span class="vote-label">Total Votes</span>
                            </div>
                            <div class="vote-breakdown">
                                <div class="vote-category">
                                    <span class="vote-icon">👍</span>
                                    <span class="vote-number">${character.votes?.like || 0}</span>
                                    <span class="vote-percent">(${character.votes?.total > 0 ? Math.round(((character.votes?.like || 0) / character.votes.total) * 100) : 0}%)</span>
                                </div>
                                <div class="vote-category">
                                    <span class="vote-icon">❤️</span>
                                    <span class="vote-number">${character.votes?.love || 0}</span>
                                    <span class="vote-percent">(${character.votes?.total > 0 ? Math.round(((character.votes?.love || 0) / character.votes.total) * 100) : 0}%)</span>
                                </div>
                                <div class="vote-category">
                                    <span class="vote-icon">⭐</span>
                                    <span class="vote-number">${character.votes?.favorite || 0}</span>
                                    <span class="vote-percent">(${character.votes?.total > 0 ? Math.round(((character.votes?.favorite || 0) / character.votes.total) * 100) : 0}%)</span>
                                </div>
                            </div>
                        </div>
                        <div class="vote-buttons">
                            <button onclick="voteForCharacter('${character.id}', 'like')" class="vote-btn like-btn">
                                <span class="vote-btn-icon">👍</span>
                                <span class="vote-btn-text">Like</span>
                            </button>
                            <button onclick="voteForCharacter('${character.id}', 'love')" class="vote-btn love-btn">
                                <span class="vote-btn-icon">❤️</span>
                                <span class="vote-btn-text">Love</span>
                            </button>
                            <button onclick="voteForCharacter('${character.id}', 'favorite')" class="vote-btn favorite-btn">
                                <span class="vote-btn-icon">⭐</span>
                                <span class="vote-btn-text">Favorite</span>
                            </button>
                        </div>
